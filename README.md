## ¡Hola Mundo! Soy Antonio Galán Soto - Desarrollador en Formación =D

> Estudiante y futuro/a desarrollador/a Full-Stack. Transformando ideas en código limpio y eficiente, con foco en el ecosistema Java y JavaScript principalmente.

---

### 🎓 Formación y Enfoque

Actualmente estoy cursando **Grado Superior DAM** y centrado en adquirir experiencia práctica en:

* **Backend Sólido:** Construcción de APIs robustas con **Spring Boot** (Java) y **Node.js**.
* **Frontend Moderno:** Desarrollo de interfaces dinámicas y escalables con **React** y **Next.js**.
* **Fundamentos:** Aprendiendo por mi cuenta en los fundamentos de la estructura de datos y algoritmos con **Python**.

---

### 🛠️ Mi Stack Tecnológico

Mi objetivo es dominar un stack versátil que me permita abordar cualquier proyecto.

| Categoría | Tecnologías Clave (Personaliza) |
| :--- | :--- |
| **Backend** | ![Java](https://img.shields.io/badge/Java-007396?style=flat&logo=openjdk&logoColor=white) ![SpringBoot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat&logo=spring-boot&logoColor=white) ![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat&logo=nodedotjs&logoColor=white) ![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white) |
| **Frontend** | ![React](https://img.shields.io/badge/React-61DAFB?style=flat&logo=react&logoColor=black) ![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat&logo=nextdotjs&logoColor=white) ![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black) ![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white) |
| **Maquetación** | ![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white) ![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white) ![Bootstrap](https://img.shields.io/badge/Bootstrap-7952B3?style=flat&logo=bootstrap&logoColor=white) |
| **Herramientas** | ![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white) ![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=flat&logo=visualstudiocode&logoColor=white) |

---

### 💡 Intereses y Colaboración

* 🌱  Mi próximo reto es consolidar sobre todas las tecnologías, frameworks y su union yt complementación **buscando ser un FullStack dev en el futuro**.
* 💻  Busco activamente **prácticas** o proyectos donde aplicar mi conocimiento en **Desarrollo de Apps Móvil | Despliegues de páginas Web]**.
* 🤝  Estoy abierto a **colaborar** en proyectos Open Source para mejorar mis habilidades de trabajo en equipo y adaptarme a trabajar en grupo.

---

### 📊 Mi Actividad en GitHub

_Muestra mi progreso y consistencia en el código._

[![Estadísticas de GitHub](https://github-readme-stats.vercel.app/api?username=TU-USUARIO-GITHUB&show_icons=true&theme=dracula&hide_border=true&count_private=true)](https://github.com/TU-USUARIO-GITHUB)

[![Principales Lenguajes](https://github-readme-stats.vercel.app/api/top-langs/?username=TU-USUARIO-GITHUB&layout=compact&theme=dracula&hide_border=true)](https://github.com/TU-USUARIO-GITHUB)

---

### 🔗 Conectemos

| Plataforma | Enlace |
| :--- | :--- |
| **LinkedIn** | [![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)]([Tu Enlace de LinkedIn]) |
